/*  MAKEFPGALOADER.C                                                   
    ...
    This code file forms a library for loading the MakeFPGA module using 
    the BackHauler PMOD.  
*/
...
DWORD MakeFPGA_Open(void);
DWORD MakeFPGA_Close(void);
DWORD MakeFPGA_ReadDeviceID(BYTE *DeviceID);
DWORD MakeFPGA_ReadUniqueID(BYTE *UniqueID);
DWORD MakeFPGA_EraseAll(void);
DWORD MakeFPGA_LoadConfiguration(void);
DWORD MakeFPGA_WriteFeatures(BYTE *FeatureRow, BYTE *Feabits);
DWORD MakeFPGA_WriteConfiguration(char *JEDECFileName);
...
#define ErrorBadUsage()             ErrorMessage("makefpgaloader <jedec file name>", "Usage")
...
/* MAIN accepts a command line argument of the jedec file to be written to
   ...
   device id codes are read and printed.  Then the device is erased and programmed
   from the specified JEDEC file.
*/
...
int main (int argc, char *argv[])
 {
  ...
  if(!MakeFPGA_Open()) return(ErrorHardware());

  // Read and print the ids
  if(!MakeFPGA_ReadDeviceID(id)) { MakeFPGA_Close(); return(-1); };
  ...
  if(!MakeFPGA_EraseAll()) { MakeFPGA_Close(); return(-1); };
  // Program FPGA 
  if(!MakeFPGA_WriteConfiguration(argv[1])){ MakeFPGA_Close(); return(-1); };
...
/* MAKEFPGA_OPEN will establish a connection to the FPGA through the chosen
...
DWORD MakeFPGA_Open(void)
 { 
  ...
  if (ret == -1) { MakeFPGA_Close(); return(0); };
...
/* MAKEFPGA_CLOSE closes the SPI port connection.
...
/* MAKEFPGA_READDEVICEID retrieves the device identification number from the
...
/* MAKEFPGA_READUNIQUEID retrieves the 8-byte unique identification code 
...
/* MAKEFPGA_ERASEALL clears the configuration from all portions of the FPGA.  
...
/* MAKEFPGA_LOADCONFIGURATION loads the active configuration from flash as
...
/* MAKEFPGA_WRITEFEATURES writes 'feature row' and 'feabits' values to the
   ...
   Also, this routine is for the MakeFPGA module, so the SPI must remain
...
/* MAKEPGA_WRITECONFIGURATION writes a JEDEC file to the configuration 
...
DWORD MakeFPGA_WriteConfiguration(char *JEDECFileName)
 {
  ...
  // All exported library functions get this check of hardware and arguments    
  // MessageBox(NULL, "MakeFPGA_WriteConfiguration", "", MB_TASKMODAL);
  if(!HWIsOpen) return(ErrorNotOpen());
  ...
  MakeFPGA_WriteFeatures(featurerow, feabits);
